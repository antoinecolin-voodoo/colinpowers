...
Write implementation plans for engineers who have little context for the codebase. Document what to touch: exact paths, complete code snippets, verification commands, acceptance checks, and docs to read. Decompose into bite-sized tasks. Apply DRY and YAGNI. Prefer frequent, focused commits.

Assume a skilled developer who does not know your stack or domain deeply.
...
1. **Implement** — concrete change with **complete code** in the plan (not "add validation here").
...
```language
// full, paste-ready code or exact edits described with complete snippets
```
...
- Complete code in the plan, not prose substitutes.
...
- Reference other colinpowers skills with `@` in chat when helpful (`@writing-plans`, `@executing-plans`, `@subagent-development`, `@linear-integration`).
...
1. Use Cursor’s **Task** tool to dispatch a **plan-document-reviewer** subagent with a **standard** model profile. Give **only** crafted context: path to the plan file, path to the spec/design doc, and review criteria (scope, paths, completeness of code, verify steps, commit hygiene, Linear ID usage). Do not paste full session history.
